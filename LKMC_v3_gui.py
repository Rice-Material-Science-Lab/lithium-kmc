...
class ElectrodepositionKMC:
    ...
    def __init__(self, params: KMCParams):
        ...
        # Build a compact 4x4 interaction lookup table.
        # Unspecified interactions remain zero.
        self.energy_lookup = np.zeros((4, 4), dtype=float)
        self.energy_lookup[FREE, DEPOSITED] = self.p.e0
        self.energy_lookup[DEPOSITED, FREE] = self.p.e0
        self.energy_lookup[DEPOSITED, DEPOSITED] = self.p.e0
        self.energy_lookup[FREE, SUBSTRATE] = self.p.e1
        self.energy_lookup[SUBSTRATE, FREE] = self.p.e1
        ...
        # We store all top-row drop events first, then all hop events.
        ...
        self._cmap = ListedColormap(["white", "tab:blue", "tab:orange", "black"])
    ...
    def _setup_indices(self) -> None:
        """Populate idx_to_data with all drop and hop event definitions."""
    ...
    def get_event_rate(self, kind: str, src: Optional[Coord], dst: Coord) -> float:
        """Return the current rate of a single drop or hop event."""
        if kind == "drop":
        ...
        # Temporarily remove the atom from the source to evaluate its energy at the destination.
        self.lattice[y0, x0] = EMPTY
        e_final = self.calc_local_energy(x1, y1, atom_type)
    ...
    def desired_bond_state(self, x: int, y: int) -> int:
        ...
        # An atom should be DEPOSITED if any nearest neighbor is substrate or deposited.
        for nx, ny in self.valid_neighbor_coords(x, y):
        ...
        self.history.append(
            {
                ...
                "free": n_free,
                "deposited": n_dep,
                "total_mobile_plus_deposited": n_total,
    ...
    def save_snapshot(self, tag: str) -> Path:
        ...
        cbar.ax.set_yticklabels(["Empty", "Free", "Deposited", "Substrate"])
    ...
    def init_gui_figure(self) -> None:
        ...
        self._im = self._ax.imshow(self.lattice_for_display(), cmap=self._cmap, vmin=0, vmax=3)
    ...
    def run_cli(self) -> None:
        ...
        while self.step < self.p.max_steps and self.time < self.p.max_time:
            ...
            # Save raw arrays periodically if requested.
...
class SimulationGUI:
    ...
    FIELD_SPECS = [
        ...
        ("e0",            float, "e0",            "Atom–atom energy e₀ (eV)",  "Interaction energy between two deposited/free atoms (eV)"),
    ...
    def _build_param_panel(self, parent: ttk.Frame) -> None:
        ...
        groups = [
            ...
            ("Physics",           self.FIELD_SPECS[2:8]),
            ("Run Controls",      self.FIELD_SPECS[8:12]),
            ("Misc",              self.FIELD_SPECS[12:14]),
        ]
    ...
    def _cmap_for_display():
        return ListedColormap(["#111111", "#5599dd", "#dd8833", "#222222"])
...
# Main entry point
# -----------------------------------------------------------------------------
# Toggle this constant to choose GUI or command-line execution.
USE_GUI = True

if __name__ == "__main__":
    if USE_GUI:
        # Create and enter the Tkinter event loop.
        root = tk.Tk()
        ...
        params = KMCParams(
            ...
            save_snapshots=True,
            save_npy_states=True,
            validation_enabled=True,
            validation_every=5000
        )

        # Build the simulator and run it without opening a GUI window.
        sim = ElectrodepositionKMC(params)
        t0 = time.time()
        sim.run_cli()
        t1 = time.time()
        print(f"Simulation complete at step {sim.step}, time {sim.time:.4e}, wall time {t1 - t0:.2f} s")